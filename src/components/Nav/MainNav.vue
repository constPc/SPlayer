...
<template>
  <nav :class="{ 'main-nav': true, 'no-sider': !showSider }">
    <div class="left">
      ...
      <!-- GitHub -->
      <!-- <Transition name="fade" mode="out-in">
        <n-button v-if="showGithub" :focusable="false" class="github" circle quaternary @click="openGithub">
    ...
    <div class="right">
      <!-- 全局菜单 -->
      <n-dropdown :show="mainMenuShow" :show-arrow="true" :options="mainMenuOptions" placement="bottom-end"
        @clickoutside="mainMenuShow = false">
        <n-button :style="{ pointerEvents: mainMenuShow ? 'none' : 'auto' }"
          :class="['main-menu', { show: !showSider }]" secondary strong round @click="mainMenuShow = !mainMenuShow">
